...
- Added a Row 5 Main Model option for Agents! note injection placement.
  ...
  - UI: Main Model Editor now has `마지막 System 뒤에 추가` and `마지막 User 뒤에 추가` button options using the existing primary/ghost button style.
  ...
  - Commits: `fc23ca4 Add main model injection target option`; `8e7ac8e Use system messages for injection placement`
