...
(async () => {
  try {
    ...
    const PAST_EVENTS_SUMMARY_TAG = 'Past Events Summary';
    ...
    async function callVertexAgent(conf, messages) {
      ...
      const accessToken = await getVertexAccessToken(conf.apiKey, conf);
      const payload = buildChatCompletionPayload(conf, messages);
    ...
    async function buildPipelineRunContext(requestMessages, chatContext, conf, pipeline) {
      ...
      const pastEventsSummary = extractPastEventsSummary(requestMessages);
    ...
    const PROMPT_BLOCK_DESCRIPTIONS = {
      'Character Description': 'Character profile and base setting.',
      'User Description': 'User persona/profile information.',
      'Author\'s Note': 'Current chat author note.',
      'Active Lorebooks': 'Lorebook entries active for this request.',
      'Global Note Replacement': 'Current character global note replacement text.',
      [PAST_EVENTS_SUMMARY_TAG]: 'RisuAI-selected long-term prior events for this current request. Use it to recover older continuity that may not appear in recent chat context.',
      'Recent Conversation': 'Recent chat messages selected for this agent.',
      'Latest Previous Assistant Response': 'The assistant response immediately before the current user input. Use it only to recover explicit prior-state values needed for continuity.',
      'Pre-Agent Notes': 'Auxiliary notes generated before the main model response.',
      'Previous Agent Notes': 'Auxiliary notes generated by earlier agents in this pipeline run.',
      'Previous Memory': 'This agent\'s saved memory from earlier turns in the current chat.',
      'Memory Instruction': 'Instructions for updating this agent\'s saved memory.',
      'Memory Format': 'Required format for this agent\'s saved memory update.',
      'Current Agent Instruction': 'The requested auxiliary analysis task.',
      'Post-processing Instruction': 'The current post-processing task.',
      'Current Response': 'The only editable target for this post-processing step.',
    };
    ...
    function normalizeAgent(agent, row, column, modelPresets = null) {
      ...
      return {
        ...
        includePastEventsSummary: agent?.includePastEventsSummary !== false,
    ...
    function buildAgentPrompt(agent, context) {
      ...
      if (agent.includePastEventsSummary && context.pastEventsSummary) {
        contextSections.push(promptSection(PAST_EVENTS_SUMMARY_TAG, context.pastEventsSummary));
      }
    ...
    function promptBlockDescription(name, group = '') {
      if (name === 'Current User Input') {
        return group === 'turn'
          ? 'The current user message that the current response is answering.'
          : 'The current user message to analyze.';
      }
      return PROMPT_BLOCK_DESCRIPTIONS[name] || 'Context block for this request.';
    }
      ...
      switch (normalizePostMode(postMode)) {
        ...
        default:
          return [
            ...
            'Use reference and immediate-turn blocks only as context. Never copy, rewrite, summarize, continue, or output content from them unless it already appears inside <Current Response>.',
            ...
            'Only change what the post-processing instruction explicitly requires; otherwise preserve the Current Response content. Do not summarize, condense, omit, expand, continue, or reinterpret it unless explicitly instructed.',
          ].join('\n');
      }
    }
    ...
    function extractTaggedBlocks(text, tag) {
      const escaped = escapeRegExp(tag);
      const pattern = new RegExp(`<\\s*${escaped}\\s*>([\\s\\S]*?)<\\/\\s*${escaped}\\s*>`, 'g');
      const blocks = [];
      const source = String(text || '');
    ...
    function extractPastEventsSummary(messages) {
      const blocks = [];
      (Array.isArray(messages) ? messages : []).forEach((message) => {
        const content = String(message?.content || '');
        if (!content.includes(PAST_EVENTS_SUMMARY_TAG)) return;
        blocks.push(...extractTaggedBlocks(content, PAST_EVENTS_SUMMARY_TAG));
      });
      return blocks.join('\n\n').trim();
    }
    ...
    function buildPreReuseKey(scope, chatContext, settingBlocks, pipeline, conf, cbsContext = null, globalNoteReplacement = '', pastEventsSummary = '') {
      ...
      const pastEventsSummaryHash = hashTextBlock(pastEventsSummary || '');
      ...
      const keyHash = createTextHasher()
        ...
        .update('pastEventsSummaryHash').update(pastEventsSummaryHash)
      ...
      for (let row = 0; row < MAIN_ROW_INDEX; row += 1) {
        ...
        agents.forEach((agent) => {
          ...
          hasher
            ...
            .update(Boolean(agent.includePastEventsSummary))
    ...
    function createPreReusedRunLog(type, pipeline, conf, scope, chatContext, settingBlocks, previousRun, preReuseKey, globalNoteReplacement = '', pastEventsSummary = '') {
    ...
    async function compactRunLogForStorage(run, debugLog) {
      ...
      await compactRunLogTextField(compacted, 'pastEventsSummary', run, 'pastEventsSummary', updatedAt, debugLog);
    ...
    async function runPrePipeline(_requestMessages, chatContext, conf, pipeline, settingBlocks, type, runScope, preReuseKey = '', runContext = null) {
      ...
      const pastEventsSummary = String(runContext?.pastEventsSummary || '').trim();
      ...
      for (let row = 0; row < MAIN_ROW_INDEX; row += 1) {
        ...
        const rowResults = await Promise.all(agents.map(async (agent) => {
          ...
          const rawPrompt = buildAgentPrompt(agent, {
            settingBlocks: settingBlocks.content,
            globalNoteReplacement,
            pastEventsSummary: runContext?.pastEventsSummary || '',
            history,
            userInput,
            notes,
    ...
    async function runPostPipeline(content, conf, pipeline, type, transientContext = null) {
      ...
      for (let row = MAIN_ROW_INDEX + 1; row < PIPELINE_ROW_COUNT; row += 1) {
        ...
        const rawPrompt = buildAgentPrompt(agent, {
          ...
          pastEventsSummary: previousRun.pastEventsSummary || '',
          history,
    ...
    function injectAgentNotes(messages, orderedNotes, mainModelConfig = null, debugLog = false) {
      if (!orderedNotes || orderedNotes.length === 0) return messages;
      const list = Array.isArray(messages) ? messages : [];
      const mainModel = normalizeMainModelConfig(mainModelConfig);
      const target = normalizeMainInjectionTarget(mainModel.injectionTarget);
      const lastRole = list.length > 0
        ? String(list[list.length - 1]?.role || '(missing)')
        : '(none)';
      if (target === MAIN_INJECTION_USER_TAIL) {
        const userIdx = findLastIndex(list, m => m.role === 'user');
        if (userIdx >= 0) {
          const injectedMessages = appendAgentNotesToMessage(list, userIdx, buildAgentNotesInjection(orderedNotes, mainModel, { userTail: true }));
          if (debugLog) logAgentInjectionPlacement(target, 'merged-into-user', list, lastRole, userIdx);
          return injectedMessages;
        }
      }

      const systemIdx = findLastIndex(list, m => m.role === 'system');
      if (systemIdx >= 0) {
        const injectedMessages = appendAgentNotesToMessage(list, systemIdx, buildAgentNotesInjection(orderedNotes, mainModel));
        const actualTarget = target === MAIN_INJECTION_USER_TAIL ? 'merged-into-system-fallback-no-user' : 'merged-into-system';
        if (debugLog) logAgentInjectionPlacement(target, actualTarget, list, lastRole, systemIdx);
        return injectedMessages;
      }

      const injectedMessages = [
        { role: 'system', content: buildAgentNotesInjection(orderedNotes, mainModel) },
        ...list,
      ];
      const actualTarget = target === MAIN_INJECTION_USER_TAIL ? 'created-leading-system-fallback-no-user' : 'created-leading-system';
      if (debugLog) logAgentInjectionPlacement(target, actualTarget, list, lastRole, 0);
      return injectedMessages;
    }
    ...
    function appendAgentNotesToMessage(messages, targetIndex, content) {
      return messages.map((message, idx) => {
        ...
        return {
          ...message,
          content: appendTextBlock(message?.content, content),
        };
      });
    }

    function appendTextBlock(existingContent, block) {
    ...
    function logAgentInjectionPlacement(requestedTarget, actualTarget, messages, lastRole, targetIndex) {
      console.log(
        `Agents! debug: main injection requested=${requestedTarget}; actual=${actualTarget}; targetIndex=${targetIndex}; messages=${Array.isArray(messages) ? messages.length : 0}; lastRole=${lastRole}`
      );
    }
...
<div class="wrap">
  ...
  <details class="card collapsible-card">
    ...
    <div class="collapsible-body">
      ...
      <div class="field">
        <label for="agents_api_timeout_seconds">Agent API Timeout (초)</label>
  ...
  <div class="actions-inner">
    ...
    function setupLiteHandlers(initialConf, initialPipeline, initialPipelineStore) {
      ...
      function agentForExport(agent) {
        ...
        exported.includePastEventsSummary = exported.includePastEventsSummary !== false;
      ...
      function renderAgentEditor() {
        ...
        root.innerHTML = `${agentEditorTitleHtml(agent)}
          ...
          <label class="checkline"><input id="edit_includePastEventsSummary" type="checkbox" ${agent.includePastEventsSummary ? 'checked' : ''}> 하이파 장기기억 포함</label>
      ...
      function renderMainModelEditor(root) {
        ...
        root.querySelectorAll('[data-main-injection-target]').forEach((button) => {
          button.addEventListener('click', () => {
            pipelineState.mainModel = normalizeMainModelConfig({
              ...pipelineState.mainModel,
              injectionTarget: button.getAttribute('data-main-injection-target'),
            });
            renderMainModelEditor(root);
          });
        });
      ...
      function bindEditorFields(agent) {
        ...
        ['enabled', 'includeSettingBlocks', 'includeGlobalNoteReplacement', 'includePastEventsSummary', 'includeHistory', 'includeUserInput', 'includePreviousNotes'].forEach((field) => {
      ...
      function previewContextForAgent(agent) {
        ...
        return {
          ...
          pastEventsSummary:
            '(실제 채팅 실행 시 RisuAI가 이번 요청에 포함한 HypaV3 과거 사건 요약이 들어갑니다)',
          history:
    ...
    async function getVertexAccessToken(text, conf = null) {
    ...
    function normalizeApiTimeoutSeconds(value, fallback = DEFAULT_AGENT_API_TIMEOUT_SECONDS) {
      const fallbackSeconds = Number.isFinite(Number(fallback))
        ? Number(fallback)
        : DEFAULT_AGENT_API_TIMEOUT_SECONDS;
      const raw = String(value ?? '').trim();
    ...
    await Risuai.addRisuReplacer('beforeRequest', async (messages, type) => {
      try {
        ...
        const preReuseKey = runLogEnabled
          ? buildPreReuseKey(runScope, chatContext, settingBlocks, pipeline, conf, runContext.cbsContext, runContext.globalNoteReplacement, runContext.pastEventsSummary)
          : '';
        ...
        if (reusableRun) {
          lastPipelineRun = createPreReusedRunLog(type, pipeline, conf, runScope, chatContext, settingBlocks, reusableRun, preReuseKey, runContext.globalNoteReplacement, runContext.pastEventsSummary);
          ...
          const injectedMessages = injectAgentNotes(messages, lastPipelineRun.notes, pipeline.mainModel, conf.debugLog);
        ...
        if (!hasUsableProviderKeyForRows(pipeline, conf, 0, MAIN_ROW_INDEX - 1)
          ...
          lastPipelineRun.pastEventsSummary = runContext.pastEventsSummary || '';
          lastPipelineRun.cbsContext = runContext.cbsContext;
          lastPipelineRun.cbsContextHash = hashAgentCbsContext(runContext.cbsContext);
        ...
        if (conf.debugLog) {
          ...
          logTextBlock('Agents! debug: Past Events Summary passed to agents', runContext.pastEventsSummary || '(not found)');
        ...
        const injectedMessages = injectAgentNotes(messages, notes, pipeline.mainModel, conf.debugLog);
        await persistRunLog(lastPipelineRun, conf.debugLog, conf.runLogEnabled);
