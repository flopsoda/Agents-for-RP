  ...
  try {
    ...
    function injectAgentNotes(messages, orderedNotes, mainModelConfig = null, debugLog = false) {
      ...
      const lastRole = Array.isArray(messages) && messages.length > 0
        ...
        : '(none)';
      const anchorRole = target === MAIN_INJECTION_USER_TAIL ? 'user' : 'system';
      const anchorIdx = findLastIndex(messages, m => m.role === anchorRole);
      const insertIndex = anchorIdx >= 0 ? anchorIdx + 1 : (Array.isArray(messages) ? messages.length : 0);
      const actualTarget = anchorIdx >= 0 ? `after-${anchorRole}` : `append-tail-no-${anchorRole}`;
      const injectedMessages = insertAgentSystemMessage(messages, insertIndex, buildAgentNotesInjection(orderedNotes, mainModel));

      if (debugLog) logAgentInjectionPlacement(target, actualTarget, messages, lastRole, insertIndex);
      return injectedMessages;
    }

    function buildAgentNotesInjection(orderedNotes, mainModelConfig = null) {
    ...
    function insertAgentSystemMessage(messages, insertIndex, content) {
    ...
    function logAgentInjectionPlacement(requestedTarget, actualTarget, messages, lastRole, insertIndex) {
      console.log(
        `Agents! debug: main injection requested=${requestedTarget}; actual=${actualTarget}; insertIndex=${insertIndex}; insertedRole=system; messages=${Array.isArray(messages) ? messages.length : 0}; lastRole=${lastRole}`
      );
    }
