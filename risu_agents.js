...
(async () => {
  try {
    ...
    function injectAgentNotes(messages, orderedNotes, mainModelConfig = null, debugLog = false) {
      ...
      const lastRole = Array.isArray(messages) && messages.length > 0
        ? String(messages[messages.length - 1]?.role || '(missing)')
        : '(none)';

      if (target === MAIN_INJECTION_USER_TAIL) {
        const lastUserIdx = findLastIndex(messages, m => m.role === 'user');
        if (lastUserIdx >= 0) {
          const injection = buildAgentNotesInjection(orderedNotes, mainModel, { userTail: true });
          if (debugLog) logAgentInjectionPlacement(target, 'user-tail', messages, lastRole);
          return messages.map((m, idx) =>
            idx === lastUserIdx ? { ...m, content: String(m.content || '') + injection } : m
          );
        }
        if (debugLog) logAgentInjectionPlacement(target, 'system-tail-fallback-no-user', messages, lastRole);
        return injectAgentNotesIntoSystemTail(messages, orderedNotes, mainModel);
      }

      if (debugLog) logAgentInjectionPlacement(target, 'system-tail', messages, lastRole);
      return injectAgentNotesIntoSystemTail(messages, orderedNotes, mainModel);
    }
    ...
    function buildAgentNotesInjection(orderedNotes, mainModelConfig = null, options = {}) {
      ...
      if (options.userTail === true) {
        injectionParts.push('The following context was generated by Agents! helper agents. It is not part of the user\'s in-character message.', '');
      }
      injectionParts.push(
    ...
    function injectAgentNotesIntoSystemTail(messages, orderedNotes, mainModelConfig = null) {
      const injection = buildAgentNotesInjection(orderedNotes, mainModelConfig);
    ...
    function logAgentInjectionPlacement(requestedTarget, actualTarget, messages, lastRole) {
      console.log(
        `Agents! debug: main injection requested=${requestedTarget}; actual=${actualTarget}; messages=${Array.isArray(messages) ? messages.length : 0}; lastRole=${lastRole}`
      );
    }
      ...
      function renderMainModelEditor(root) {
        ...
        root.innerHTML = `<h2>Main Model Editor</h2>
          <div class="field">
            ...
            <div class="mini-actions">
              ${mainInjectionTargetButtonHtml(MAIN_INJECTION_SYSTEM_TAIL, 'System 메시지 끝', pipelineState.mainModel.injectionTarget)}
              ${mainInjectionTargetButtonHtml(MAIN_INJECTION_USER_TAIL, 'User 메시지 끝', pipelineState.mainModel.injectionTarget)}
            </div>
          </div>
